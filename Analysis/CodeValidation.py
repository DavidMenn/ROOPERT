...
def fixedRSquareChanelSetup(params,xlist, rlist,chlist,chanelToLandRatio,twlist,nlist,helicitylist = None,dxlist = None):# MAKE SURE TO PASS SHIT  ALREADY FLIPPED
    ...
    heatingfactorlist = np.ones(xlist.size)*.2 #.3 # .6 is from cfd last year, i think its bs but whatever
...
TC ):
    ...
    if sattemp < 293*const.degKtoR:
        inittemp=sattemp/const.degKtoR
    else:
        inittemp = 293
    Twglist, hglist, Qdotlist, Twclist, hclist, Tclist, coolantpressurelist, qdotlist, fincoolingfactorlist, rholist, viscositylist, Relist = CS.steadyStateTemperatures(
        ...
        heatingfactorlist, xlist, vlistflipped, inittemp, params['pc'] + params['pc']*.2 + 50*const.psiToPa, twlistflipped, hydraulicdiamlist, rgaslist = rlist, fincoolingfactorfunc=fincoolingfactorfunc, dxlist = dxlist)
...
args = {
        'thrust': 2450000 ,  # Newtons
        # 'rho_ox' : 1141, #Kg/M^3
        # 'rho_fuel' : 842,
        'pc': 13400000,
        'pe': 14.7 * const.psiToPa,
       # 'phi':1,
        'cr': 1.6, #idk this is a guess
        'P_tank_ox' : 1e6,
        'P_tank_fuel': 1e6,
...
args['rm']=mrideal
configtitle = "BE4 New Glenn Validation"
...
xlist = np.linspace(0, params['lc'] + (params['rc'] - params['rt']) / math.tan(params['thetac']) + params['ln_conical'], 4*120)
...
rlist,xlist = RListGenerator.paraRlist(xlist, params['lc'], params['rc'],
                                  ...
                                  params['re'], params['lc']/5, 1.5*2*.0254, .4*2*.0254, math.pi/6 , 8*math.pi/180, params['er'])  # xlist, xns, rc, xt, rt, xe, re
...
twlist  = (rlistflipped/TC.rt)*.0005 
nlist  = np.ones(len(xlist))*180
ewlist  = chlist*2
...
path=os.path.join( "Configs","Bean_CAD_Validation")
...
xlistnew, ylistnew,zlistnew = CAD.ChanelBoxCorners(xlist,rlist,np.flip(twlist),
                helicitylist, np.flip(chlist), np.flip(cwlist)) 
path=os.path.join( "Configs","Validation_CAD")
os.makedirs(path,exist_ok=True)
